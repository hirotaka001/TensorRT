...
//値は、TRT_YOLOv4.cppでセットする。yololayerのプラグインは、tensorRTの中でインスタンスが動的に作られる為、
...
namespace nvinfer1
{
	YoloLayerPlugin::YoloLayerPlugin()
	{
		...
		mKernelCount = (int)mYoloKernel.size();
	}
	...
	YoloLayerPlugin::YoloLayerPlugin(const void* data, size_t length)
	{
		...
		assert(d == a + length);
		if (d == a + length) {
			// warning : variable "a" was declared but never referenced 対策
		}
	}
	...
	__global__ void CalDetection(const float* input, float* output, int noElements,
		int yoloWidth, int yoloHeight, const float anchors[CHECK_COUNT * 2], int classes, int outputElem, int width, int height) {
		...
		for (int k = 0; k < 3; ++k) {
			...
			//Location
			det->bbox[0] = (col + Logist(curInput[idx + k * info_len_i * total_grid + 0 * total_grid])) * width / yoloWidth;
			det->bbox[1] = (row + Logist(curInput[idx + k * info_len_i * total_grid + 1 * total_grid])) * height / yoloHeight;
	...
	void YoloLayerPlugin::forwardGpu(const float* const* inputs, float* output, cudaStream_t stream, int batchSize) {
		...
		for (unsigned int i = 0; i < mYoloKernel.size(); ++i)
		{
			...
			CalDetection << < (yolo.width * yolo.height * batchSize + mThreadCount - 1) / mThreadCount, mThreadCount >> >
				(inputs[i], output, numElem, yolo.width, yolo.height, (float*)devAnchor, mClassCount, outputElem, g_width, g_height);
		}
