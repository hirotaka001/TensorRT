...
int main(int argc, char** argv)
{
	...
	if (!cTRT_YOLOv4.Init(gpu_id, model_filename, model_dir, classes, width, height, batches, super_batches, threads, is_pre_process_cuda,
	...
	cTRT_YOLOv4.InferParallel(loaded_input_images, detection_result);
	...
	if (draw_flag)
	{
		...
		for (int Image_idx = 0; Image_idx < num_images; Image_idx++)
		{
			...
			kros::TRT_YOLOv4::DrawResult(tmp, detection_result[Image_idx], label_list);
