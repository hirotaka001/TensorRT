...
int main(int argc, char** argv)
{
	...
	int gpuid = parser.get<int>("GPUID");
...
//	std::vector<std::string> output_layers{ "082_convolutional", "094_convolutional", "106_convolutional" };
	kros::TRT_YOLOv4 cTRT_YOLOv4;
	if (!cTRT_YOLOv4.Init(gpuid, model_filename, model_dir, classes, width, height, batches, super_batches, threads, is_pre_process_cuda, score_threshold, nms_threshold/*, input_layers, output_layers*/)) {
		exit(EXIT_FAILURE);
	}
	...
	for (int thread_idx = 0; thread_idx < threads; thread_idx++) {
		...
		cTRT_YOLOv4.Infer(input_images[0][thread_id], detection_result_[0][thread_id], thread_id);
...
#pragma omp parallel for num_threads(threads) schedule(static)
		for (int thread_idx = 0; thread_idx < threads; thread_idx++) {
			int thread_id = (int)omp_get_thread_num();
			cTRT_YOLOv4.Infer(input_images[loop_idx][thread_id], detection_result_[loop_idx][thread_id], thread_id);
		}
		...
		for (int loop_idx = 0; loop_idx < loops; loop_idx++) {
			for (int thread_idx = 0; thread_idx < threads; thread_idx++) {
				...
				for (int i = 0; i < batches * super_batches; i++) {
					...
					kros::TRT_YOLOv4::DrawResult(tmp, detection_result_[loop_idx][thread_idx][i], label_list);
